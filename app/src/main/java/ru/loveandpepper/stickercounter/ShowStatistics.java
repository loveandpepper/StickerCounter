...
public class ShowStatistics {
    ...
    public void collectAndShow(TextView textView, TextView marginal, List<Product> list, int usdForStats, PieChartView pieChartView){
        StringBuilder stb = new StringBuilder();
        int allsells = list.stream().map(Product::getTotal).reduce(0,Integer::sum);
        textView.append(String.format(" всего на сумму %d руб.\n-------", allsells));
        ...
        int win7keytotal = win7keyprice - (300*win7keyquantity);
        ...
        int officekeytotal = officekeyprice - (610*officekeyquantity);
        ...
        textView.append(stb);
        int marginalForMonth = (win10coatotal + win7coatotal + win10keytotal + win7keytotal + officekeytotal + otherprice);
        marginal.setText(String.format("Всего месяц (без рекламы) чистыми ~ %d руб.", marginalForMonth));

        pieChartDrawer(win10coaprice, win7coaprice, win10keyprice, win7keyprice, officekeyprice, otherprice, pieChartView, allsells);
    }

    public void pieChartDrawer(int win10coasells, int win7coasells, int w10keysells, int w7keysells, int office, int other, PieChartView pcw, int allsells){
        ...
        pieChartData.setHasCenterCircle(true).setHasLabels(true).setValueLabelTextSize(10);
